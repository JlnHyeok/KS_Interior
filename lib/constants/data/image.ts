export const bannerDeskImgInfo = [
  { src: "/img/banner/first_banner.jpeg", alt: "first_banner" },
  { src: "/img/banner/second_banner.jpeg", alt: "second_banner" },
  { src: "/img/banner/third_banner.jpeg", alt: "third_banner" },
  { src: "/img/banner/fourth_banner.jpeg", alt: "fourth_banner" },
]
export const bannerMobileImgInfo = [
  { src: "/img/mobile/banner/first_banner.png", alt: "first_banner" },
  { src: "/img/mobile/banner/second_banner.png", alt: "second_banner" },
  { src: "/img/mobile/banner/third_banner.png", alt: "third_banner" },
  { src: "/img/mobile/banner/fourth_banner.png", alt: "fourth_banner" },
]
export const bannerInnerText = [
  {
    title: "브라운 내추럴 디자인",
    content:
      "거실 인테리어는 곧 집의 인테리어를 결정합니다.\n중심을 잡아주는 내추럴 디자인",
  },
  {
    title: "화이트 & 우드",
    content:
      "주방을 더욱 따뜻한 공간으로 만들어 줄\nKS인테리어만의 화이트&우드 인테리어",
  },
  {
    title: "모던 인테리어",
    content: "톤 다운된 컬러들의 조화가 엿보이는\nKS인테리어만의 모던 인테리어",
  },
  {
    title: "사무실 인테리어",
    content:
      "회의실과 임원실을 더 세련되고 미니멀하게\nKS인테리어만의 사무실 인테리어",
  },
]

export const reviewImgInfo = [
  {
    src: "/img/review/dummy_1.png",
    alt: "first_review",
    content:
      "제가 원했던 느낌대로 시공해주시려고 노력하는 모습에 너무 좋았습니다 ㅎㅎ",
    title: "꼼꼼하게 시공해주셔서 너무 너무 감사합니다.",
    subTitle: "트렌디 화이트",
    score: 5,
    date: "2023 - 03 - 12",
  },
  {
    src: "/img/review/dummy_2.png",
    alt: "second_review",
    title: "평소에 고민하던 거실 인테리어를 해결해주셔서 감사합니다.",
    content:
      "평소에 고민하던 거실 인테리어를 새롭게 바꾸고 나니까 거실 분위기가 더 잘 사는거같아요. 만족합니다",
    subTitle: "트렌디 화이트",
    score: 4,
    date: "2023 - 04 - 20",
  },
  {
    src: "/img/review/dummy_3.png",
    alt: "third_review",
    title: "전체적으로 톤을 맞춰주셔서 너무 만족스럽습니다. 훨씬 깔끔해졌어요.",
    content:
      "톤을 맞추고 싶어서 의뢰했어요~ 너무 예쁘게 잘 해주셔서 완전 만족이구요 제가 원하는 느낌으로 딱 잘 됐어요.",
    subTitle: "트렌디 화이트",
    score: 5,
    date: "2023 - 04 - 23",
  },
  {
    src: "/img/review/dummy_4.png",
    alt: "fourth_review",
    title: "이번에 사무실 이전하면서 새로 배치를 바꾸고 싶어서 의뢰했어요. ",
    content:
      "사무실을 이전하면서 새롭게 인테리어를 했는데 생각한것보다 더 좋은 방향으로 바뀌게 된거같아서 너무 좋아요.",
    subTitle: "트렌디 화이트",
    score: 5,
    date: "2023 - 05 - 12",
  },
]
